...
def load_baseline_life_ranges(path: Optional[str], baseline_life: Dict[str, float]) -> pd.DataFrame:
    """Lädt artspezifische Lebensdauer-Bandbreiten.

    Erwartete CSV-Spalten: species_norm, life_min, life_mode, life_max.
    Ohne Datei werden aus den fixen Baseline-Werten degenerierte Bandbreiten
    min=mode=max erzeugt. Dadurch bleibt das Modell abwärtskompatibel.
    """
    rows = []
    for sp, val in baseline_life.items():
        rows.append({"species_norm": normalize_species_name(sp), "life_min": val, "life_mode": val, "life_max": val})
    ...
    t = read_table(path)
    required = ["species_norm", "life_min", "life_mode", "life_max"]
    missing = [c for c in required if c not in t.columns]
    if missing:
        raise ValueError(f"Baseline-Life-Range CSV braucht Spalten {required}; fehlend: {missing}")

    t = t[required].copy()
    t["species_norm"] = t["species_norm"].apply(normalize_species_name)
    for c in ["life_min", "life_mode", "life_max"]:
        t[c] = pd.to_numeric(t[c], errors="coerce")

    t = t.dropna(subset=["species_norm"])
    t = t[t["species_norm"] != "unbekannt"].copy()
    t["life_mode"] = t["life_mode"].where(t["life_mode"].notna(), t[["life_min", "life_max"]].mean(axis=1))
    t["life_min"] = t["life_min"].where(t["life_min"].notna(), t["life_mode"])
    t["life_max"] = t["life_max"].where(t["life_max"].notna(), t["life_mode"])
    low = t[["life_min", "life_mode", "life_max"]].min(axis=1)
    high = t[["life_min", "life_mode", "life_max"]].max(axis=1)
    mode = t["life_mode"].clip(lower=low, upper=high)
    t["life_min"], t["life_mode"], t["life_max"] = low, mode, high
    t = t[(t["life_min"] > 0) & (t["life_mode"] > 0) & (t["life_max"] > 0)]

    merged = base.merge(t, on="species_norm", how="outer", suffixes=("_default", "_csv"))
    for c in ["life_min", "life_mode", "life_max"]:
        merged[c] = merged[f"{c}_csv"].combine_first(merged[f"{c}_default"])
    return merged[["species_norm", "life_min", "life_mode", "life_max"]]


def attach_life_ranges(sim_input: pd.DataFrame, life_ranges: pd.DataFrame) -> pd.DataFrame:
    """Hängt life_min/life_mode/life_max an sim_input an und füllt Lücken robust."""
    out = sim_input.copy()
    lr = life_ranges.copy()
    lr["species_norm"] = lr["species_norm"].apply(normalize_species_name)
    out = out.merge(lr, on="species_norm", how="left")

    fallback = pd.to_numeric(out.get("avg_life_baseline"), errors="coerce")
    median_fb = float(np.nanmedian(fallback)) if np.isfinite(fallback).any() else 100.0
    fallback = fallback.fillna(median_fb)
    for c in ["life_min", "life_mode", "life_max"]:
        out[c] = pd.to_numeric(out[c], errors="coerce").fillna(fallback)

    low = out[["life_min", "life_mode", "life_max"]].min(axis=1)
    high = out[["life_min", "life_mode", "life_max"]].max(axis=1)
    mode = out["life_mode"].clip(lower=low, upper=high)
    out["life_min"], out["life_mode"], out["life_max"] = low, mode, high
    out["avg_life_baseline"] = out["life_mode"]
    return out


def sample_life_values(
    rng: np.random.Generator,
    species_arr: np.ndarray,
    life_min: np.ndarray,
    life_mode: np.ndarray,
    life_max: np.ndarray,
    *,
    life_sampling: str,
    life_uncertainty_mode: str,
    deterministic: bool,
) -> np.ndarray:
    """Erzeugt Lebensdauerannahmen pro Baum für die Hazard-Anpassung."""
    left0 = np.asarray(life_min, dtype=float)
    mode0 = np.asarray(life_mode, dtype=float)
    right0 = np.asarray(life_max, dtype=float)
    left = np.minimum.reduce([left0, mode0, right0])
    right = np.maximum.reduce([left0, mode0, right0])
    mode = np.clip(mode0, left, right)

    if deterministic or life_uncertainty_mode == "none" or life_sampling == "mode":
        return mode.copy()

    if life_uncertainty_mode == "per_tree":
        vals = mode.copy()
        nondeg = right > left
        if nondeg.any():
            if life_sampling == "uniform":
                vals[nondeg] = rng.uniform(left[nondeg], right[nondeg])
            else:
                vals[nondeg] = rng.triangular(left[nondeg], mode[nondeg], right[nondeg])
        return vals

    if life_uncertainty_mode == "per_run":
        vals = mode.copy()
        for sp in pd.unique(pd.Series(species_arr)):
            idx = np.where(species_arr == sp)[0]
            if len(idx) == 0:
                continue
            l = float(np.nanmedian(left[idx])); m = float(np.nanmedian(mode[idx])); r = float(np.nanmedian(right[idx]))
            l, r = min(l, m, r), max(l, m, r)
            m = min(max(m, l), r)
            if np.isclose(l, r):
                draw = m
            elif life_sampling == "uniform":
                draw = float(rng.uniform(l, r))
            else:
                draw = float(rng.triangular(l, m, r))
            vals[idx] = draw
        return vals

    raise ValueError("life_uncertainty_mode muss none, per_run oder per_tree sein")


def make_life_hazard_multiplier(
    life_values: np.ndarray,
    reference_life: float,
    weight: float,
    min_multiplier: float,
    max_multiplier: float,
) -> np.ndarray:
    """Konvertiert Lebensdauerannahmen in einen Hazard-Multiplikator.

    Langlebige Arten: multiplier < 1; kurzlebige Arten: multiplier > 1.
    weight=0 deaktiviert den Effekt.
    """
    if weight <= 0:
        return np.ones(len(life_values), dtype=float)
    ref = float(reference_life) if np.isfinite(reference_life) and reference_life > 0 else float(np.nanmedian(life_values))
    life = np.asarray(life_values, dtype=float)
    life = np.where(np.isfinite(life) & (life > 0), life, ref)
    mult = np.power(ref / life, weight)
    return np.clip(mult, min_multiplier, max_multiplier)
...
def run_monte_carlo(
    ...
    *,
    ...
    new_tree_initial_age: int,
    new_tree_strategy: str,
    target_count: Optional[int],
    life_sampling: str = "triangular",
    life_uncertainty_mode: str = "none",
    life_hazard_adjustment_weight: float = 0.0,
    life_hazard_reference: float = 0.0,
    life_hazard_min_multiplier: float = 0.5,
    life_hazard_max_multiplier: float = 2.0,
    ...
    # Neue Pflanzungen/Ersatzpflanzungen sind in der Praxis nicht 0 Jahre alt,
    # sondern kommen typischerweise als vorgezogene Jungbäume aus der Baumschule.
    # Da die Simulation im Jahresblock nach dem Einfügen aller neuen Bäume einmal altert,
    # wird intern ein Jahr abgezogen. So werden frisch gepflanzte 10-jährige Bäume
    # im ersten simulierten Risiko-Jahr mit Alter 10 behandelt.
    planting_age_internal = int(np.clip(new_tree_initial_age - 1, 0, max_age))

    life_min_base = pd.to_numeric(sim_input.get("life_min"), errors="coerce").to_numpy(float)
    life_mode_base = pd.to_numeric(sim_input.get("life_mode"), errors="coerce").to_numpy(float)
    life_max_base = pd.to_numeric(sim_input.get("life_max"), errors="coerce").to_numpy(float)
    if len(life_mode_base) != len(sim_input):
        life_mode_base = pd.to_numeric(sim_input.get("avg_life_baseline"), errors="coerce").to_numpy(float)
        life_min_base = life_mode_base.copy()
        life_max_base = life_mode_base.copy()
    life_ref_auto = float(np.nanmedian(life_mode_base)) if np.isfinite(life_mode_base).any() else 100.0
    life_reference_used = life_hazard_reference if life_hazard_reference and life_hazard_reference > 0 else life_ref_auto
    ...
    for run in range(n_runs):
        ...
        life_values_base_run = sample_life_values(
            rng, species_arr_base, life_min_base, life_mode_base, life_max_base,
            life_sampling=life_sampling,
            life_uncertainty_mode=life_uncertainty_mode,
            deterministic=deterministic,
        )
        life_mult_base = make_life_hazard_multiplier(
            life_values_base_run,
            life_reference_used,
            life_hazard_adjustment_weight,
            life_hazard_min_multiplier,
            life_hazard_max_multiplier,
        )
        life_mult = life_mult_base.copy()
            ...
            if y > 0:
                ...
                if due:
                    for item in due:
                        ...
                        age = np.concatenate([age, np.full(n_new, planting_age_internal, dtype=int)])
                        ...
                        life_mult = np.concatenate([life_mult, np.full(n_new, item.get("life_mult", 1.0))])
                ...
                if annual_new_trees > 0 and annual_new_trees_start_offset <= y <= end_offset:
                    ...
                    age = np.concatenate([age, np.full(n_new_planted, planting_age_internal, dtype=int)])
                    ...
                    life_mult = np.concatenate([life_mult, life_mult_base[pick]])
                ...
                if n_deaths > 0 and replacement_rate > 0:
                    ...
                    if n_repl > 0:
                        ...
                        repl_df = pd.DataFrame({
                            ...
                            "life_mult": life_mult[repl_idx],
                        })
                        grouped = repl_df.groupby(
                            ["species", "category", "climate_mult", "site_mult", "management_mult", "urban_mult", "category_mult", "life_mult"],
                            dropna=False,
                        ...
                        for _, row in grouped.iterrows():
                            replacement_queue.append({
                                ...
                                "life_mult": float(row.get("life_mult", 1.0)),
                                "n": int(row["n"]),
                            })
            else:
                n_deaths = 0
...
def main() -> None:
    ...
    ap.add_argument("--new_tree_initial_age", type=int, default=10, help="Alter frisch gepflanzter Ersatz- und Zusatzbäume in Jahren. Standard: 10 Jahre.")
    ...
    ap.add_argument("--baseline_life_range_csv", default=None, help="Optional: CSV mit species_norm, life_min, life_mode, life_max für Lebensdauer-Bandbreiten.")
    ap.add_argument("--life_sampling", default="triangular", choices=["mode", "triangular", "uniform"], help="Wie aus life_min/life_mode/life_max gezogen wird.")
    ap.add_argument("--life_uncertainty_mode", default="none", choices=["none", "per_run", "per_tree"], help="none=Mode; per_run=pro Art und MC-Lauf; per_tree=pro Baum ziehen.")
    ap.add_argument("--life_hazard_adjustment_weight", type=float, default=0.0, help="Stärke der Lebensdauer-Anpassung auf die Hazard. 0=aus, 0.3=schwach, 0.5=mittel, 1.0=stark.")
    ap.add_argument("--life_hazard_reference", type=float, default=0.0, help="Referenz-Lebensdauer für Hazard-Anpassung. 0=Median der verwendeten Arten.")
    ap.add_argument("--life_hazard_min_multiplier", type=float, default=0.5, help="Untergrenze des Lebensdauer-Hazardmultiplikators.")
    ap.add_argument("--life_hazard_max_multiplier", type=float, default=2.0, help="Obergrenze des Lebensdauer-Hazardmultiplikators.")
    ...
    life_ranges = load_baseline_life_ranges(args.baseline_life_range_csv, baseline_life)
    sim_input = attach_life_ranges(sim_input, life_ranges)
    ...
    total_long, annual_long, species_long = run_monte_carlo(
        ...
        new_tree_initial_age=args.new_tree_initial_age,
        new_tree_strategy=args.new_tree_strategy,
        target_count=args.target_count,
        life_sampling=args.life_sampling,
        life_uncertainty_mode=args.life_uncertainty_mode,
        life_hazard_adjustment_weight=args.life_hazard_adjustment_weight,
        life_hazard_reference=args.life_hazard_reference,
        life_hazard_min_multiplier=args.life_hazard_min_multiplier,
        life_hazard_max_multiplier=args.life_hazard_max_multiplier,
    ...
    meta = pd.DataFrame(
        [
            ...
            ["baseline_life_range_csv", args.baseline_life_range_csv],
            ["life_sampling", args.life_sampling],
            ["life_uncertainty_mode", args.life_uncertainty_mode],
            ["life_hazard_adjustment_weight", args.life_hazard_adjustment_weight],
            ["life_hazard_reference", args.life_hazard_reference],
