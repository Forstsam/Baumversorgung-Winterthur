...
const SPECIES_BASE=[
  ...
  {"name": "Andere Arten", "label": "Andere Arten", "count": 4298, "fallen": 3035, "avgAge": 33.0, "lifeMin": 89, "life": 128, "lifeMax": 174, "climateFit": 0.54, "risk": 0.95},
];
  ...
  return(<>
    <div style={{background:PANEL,border:`1px solid ${LINE_GRID}`,borderRadius:16,padding:"18px 14px 8px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",padding:"0 8px 8px",gap:10,flexWrap:"wrap"}}>
        ...
        <div style={{fontSize:11,color:TEXT_MUTE,fontFamily:"var(--mono)"}}>Top 10 · deterministische Artenschicht</div>
      </div>
      <ResponsiveContainer width="100%" height={360}>
        <ComposedChart data={species.chart} layout="vertical" margin={{top:8,right:18,bottom:4,left:94}}>
          ...
          <YAxis type="category" dataKey="art" width={90} tick={{fill:TEXT_MUTE,fontSize:11}} stroke={LINE_GRID}/>
          <Tooltip contentStyle={{background:"#fff",border:`1px solid ${LINE_GRID}`,borderRadius:9,fontFamily:"var(--mono)",fontSize:12}}/>
    ...
    <div style={{marginTop:14,background:CARD,border:`1px solid ${LINE_GRID}`,borderRadius:12,padding:"14px 18px"}}>
      <div style={{fontSize:13,color:SAGE,fontWeight:600,marginBottom:8}}>Relative Veränderung der wichtigsten Artengruppen</div>
      <div style={{display:"grid",gridTemplateColumns:"1.2fr .55fr .55fr .55fr .55fr .55fr",gap:8,fontSize:11,color:TEXT_MUTE,fontFamily:"var(--mono)",borderBottom:`1px solid ${LINE_GRID}`,paddingBottom:6,marginBottom:6}}>
        <div>Artengruppe</div><div>2026</div><div>{year}</div><div>Δ Anteil</div><div>Alter</div><div>Leben</div>
      </div>
      {species.trend.map(r=>(<div key={r.art} style={{display:"grid",gridTemplateColumns:"1.2fr .55fr .55fr .55fr .55fr .55fr",gap:8,fontSize:12,padding:"5px 0",borderBottom:`1px solid ${LINE_GRID}55`}}>
        <div style={{color:PAPER}}>{r.art}</div><div style={{fontFamily:"var(--mono)",color:SAGE}}>{r.heute.toFixed(1)} %</div><div style={{fontFamily:"var(--mono)",color:MOSS}}>{r.jahr.toFixed(1)} %</div><div style={{fontFamily:"var(--mono)",color:r.delta>=0?GREEN:BRICK}}>{r.delta>0?"+":""}{r.delta.toFixed(1)} %-P</div><div style={{fontFamily:"var(--mono)",color:SAGE}}>{r.avgAge} J</div><div style={{fontFamily:"var(--mono)",color:SAGE}}>{r.life} J</div>
      </div>))}
      <div style={{fontSize:11,color:TEXT_MUTE,lineHeight:1.5,marginTop:10}}>{SPECIES_NOTE} Die Artenschicht verteilt Ausfälle proportional nach katasterbasiertem Risiko und Lebensdauer-Proxy; Pflanzungen folgen der gewählten Strategie. Für finale Modelltreue sollten species_summary.csv/species_milestones.csv aus dem Python-Lauf verwendet werden.</div>
    </div>
  </>);
}
